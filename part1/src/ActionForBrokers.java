    ...
    public void run() {
        ...
        try {
            try {
                ...
                if (message.equals("C")) {
                    try {
                        message = (String) in.readObject(); //username
                        publisherport= (String) in.readObject(); //port
                        publisherip=(String) in.readObject(); //ip
                        String gcname=(String)in.readObject();//gc
                        broker.pull(Integer.parseInt(publisherport), out,publisherip);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    ...
                    broker.gc=new GroupChat(tempPublisher[1],newport);
